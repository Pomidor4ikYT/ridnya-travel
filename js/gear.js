(function() {
  ...
  const defaultGear = [
    { id: 'g1', title: 'Новий намет Mountain Hardwear', date: '20.04.2026', text: 'Легкий двомісний намет для весняних походів. Вага всього 1.8 кг.', image: 'images/img11.jpg' },
    { id: 'g2', title: 'Трекінгові палиці Black Diamond', date: '15.04.2026', text: 'Складані, алюмінієві, з пробковою ручкою.', image: 'images/img1.jpg' }
  ];
  ...
  function renderGear() {
    ...
    if (!container) return;
    if (!gearItems.length) { container.innerHTML = '<div style="text-align:center; padding:50px;"><i class="fas fa-campground" style="font-size:3rem; opacity:0.4; display:block; margin-bottom:16px;"></i><p>Немає записів</p></div>'; return; }
    container.innerHTML = gearItems.map(item => `
      <article class="blog-card" data-id="${item.id}">
        ...
        <div class="blog-card-body"><span class="blog-date">${item.date || ''}</span><h4>${item.title}</h4><p>${item.text || ''}</p></div>
      </article>`).join('');
  ...
  function openGearModal(id = null) {
    ...
    if (id) { const item = gearItems.find(p => p.id === id); if (!item) return; editingGearId = id; modalTitle.innerHTML = '<i class="fas fa-pen"></i> Редагувати запис'; document.getElementById('gearTitle').value = item.title; document.getElementById('gearDate').value = item.date || ''; document.getElementById('gearText').value = item.text || ''; document.getElementById('gearImage').value = item.image || ''; }
    else { editingGearId = null; modalTitle.innerHTML = '<i class="fas fa-campground"></i> Новий запис'; form.reset(); }
  ...
  function init() {
    ...
    document.getElementById('addGearForm')?.addEventListener('submit', (e) => {
      e.preventDefault();
      const title = document.getElementById('gearTitle').value.trim(); if (!title) { alert('Назва обовʼязкова'); return; }
      const data = { title, date: document.getElementById('gearDate').value.trim(), text: document.getElementById('gearText').value.trim(), image: document.getElementById('gearImage').value.trim() };
      if (editingGearId) { const idx = gearItems.findIndex(p => p.id === editingGearId); if (idx !== -1) gearItems[idx] = { ...gearItems[idx], ...data }; }
      else { gearItems.push({ id: 'g' + Date.now(), ...data }); }
      saveData(); renderGear(); closeGearModal();
    });
  }
