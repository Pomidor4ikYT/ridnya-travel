...
(function() {
  ...
  const defaultGear = [
    { id: 'g1', title: 'Намет Mountain Hardwear Strato', date: '20.04.2026', text: 'Легкий двомісний намет...', image: 'https://propohody.com/wp-content/uploads/2025/02/%D0%97%D0%B0%D1%81%D1%82%D0%B0%D0%B2%D0%BA%D0%B0-%D0%BA%D0%BE%D0%BF%D1%96%D1%8F3.jpg' },
    { id: 'g2', title: 'Трекінгові палиці Black Diamond Trail', date: '15.04.2026', text: 'Складані, алюмінієві...', image: 'https://propohody.com/wp-content/uploads/2017/07/20170718_171603.jpg' },
    { id: 'g3', title: 'Рюкзак Osprey Kestrel 48L', date: '10.04.2026', text: 'Універсальний туристичний рюкзак...', image: 'https://propohody.com/wp-content/uploads/2019/06/IMG_20190602_155742.jpg' }
  ];
  let gearItems = [];
  ...
  function loadData() { gearItems = Utils.getStorage(STORAGE_KEY, defaultGear); }
  function saveData() { Utils.setStorage(STORAGE_KEY, gearItems); }
  ...
  function attachGearEvents() {
    if (window.isAdmin) {
      document.querySelectorAll('#gearContainer .edit-btn').forEach(btn => btn.addEventListener('click', (e) => { e.stopPropagation(); openGearModal(btn.dataset.id); }));
      document.querySelectorAll('#gearContainer .delete-btn').forEach(btn => btn.addEventListener('click', (e) => { e.stopPropagation(); if (confirm('Видалити запис?')) { gearItems = gearItems.filter(p => p.id !== btn.dataset.id); saveData(); renderGear(); } }));
    }
  }
