...
(function() {
  ...
  const defaultTrips = [
    ...
    { id: '17', title: "г.Петрос 2020 м (з с.Кваси)", date: "2026-12-05", distance: "26 км", difficulty: "вище середньої", duration: "2 дні", guide: "Петро Маковський", report: "", mapUrl: "https://uk.mapy.cz/s/ghijkl", notes: "", image: "https://we.org.ua/wp-content/uploads/2015/03/211.jpg", isTraditional: false, createdAt: Date.now() - 60 * 86400000, history: "" }
  ];
  ...
  function loadData() {
    Promise.all([
      ...
      Utils.fetchData('members'),
      Utils.fetchData('trip_applications'),
      Utils.fetchData('leader_applications')
    ]).then(([tripsData, membersData, tripApps, leaderApps]) => {
      trips = (Array.isArray(tripsData) && tripsData.length) ? tripsData : defaultTrips;
      trips.forEach(t => { if (t.history === undefined) t.history = ''; });
      members = (Array.isArray(membersData) && membersData.length) ? membersData : defaultMembers;
      window._tripApps = Array.isArray(tripApps) ? tripApps : [];
      window._leaderApps = Array.isArray(leaderApps) ? leaderApps : [];
      renderAll();
    }).catch(err => {
      console.error('Помилка завантаження даних:', err);
      Utils.showNotification('Помилка завантаження даних. Спробуйте оновити сторінку.', false);
      trips = defaultTrips;
  ...
  // ========== ЗБЕРЕЖЕННЯ ==========
  function saveTrips() { Utils.saveData('trips', trips); }
  function saveMembers() { Utils.saveData('members', members); }
  function saveTripApps() { Utils.saveData('trip_applications', window._tripApps); }
  function saveLeaderApps() { Utils.saveData('leader_applications', window._leaderApps); }
  ...
  function sortTrips(tripsArray, sortType, ascending) {
    ...
    if (sortType === 'date') {
      const future = [], past = [], noDate = [];
      const today = new Date(); today.setHours(0,0,0,0);
      sorted.forEach(trip => {
        const dateObj = parseDate(trip.date);
        if (!dateObj) noDate.push(trip);
        else if (dateObj >= today) future.push(trip);
        else past.push(trip);
      });
  ...
  // ========== РОБОТА З УЧАСНИКАМИ ==========
  function getTripApplications(tripId) {
    return (window._tripApps || []).filter(a => a.tripId == tripId);
  }

  function getApprovedParticipantsForTrip(tripId) {
    return getTripApplications(tripId).filter(a => a.status === 'approved');
  }
  ...
  // ===== ВИПРАВЛЕНА ПАГІНАЦІЯ – стрілки ліворуч і праворуч =====
  function renderPagination(totalPages) {
    ...
    html += `<button class="page-nav" id="tripsPrevPage" ${currentPage === 1 ? 'disabled' : ''}><i class="fas fa-chevron-left"></i></button>`;
    html += '<div class="page-numbers">';
    for (let i = 1; i <= totalPages; i++) {
      html += `<button class="page-number ${i === currentPage ? 'active' : ''}" data-page="${i}">${i}</button>`;
    }
    html += '</div>';
    html += `<button class="page-nav" id="tripsNextPage" ${currentPage === totalPages ? 'disabled' : ''}><i class="fas fa-chevron-right"></i></button>`;
    html += '</div>';
    container.innerHTML = html;

    document.getElementById('tripsPrevPage')?.addEventListener('click', function() {
      if (currentPage > 1) { currentPage--; renderTrips(); }
    });
    document.getElementById('tripsNextPage')?.addEventListener('click', function() {
      if (currentPage < totalPages) { currentPage++; renderTrips(); }
    });
    document.querySelectorAll('#tripsPagination .page-number').forEach(btn => {
  ...
  function renderTrips() {
    ...
    const pageTrips = filtered.slice(start, start + ITEMS_PER_PAGE);
    ...
    container.innerHTML = pageTrips.map(trip => {
      const isCompleted = isTripCompleted(trip);
      const isNew = (trip.createdAt && (Date.now() - trip.createdAt < 30 * 86400000));
      ...
      if (!isCompleted && isUpcomingSoon(trip)) {
        ...
        badgeHtml = '<span class="trip-badge new">Новинка</span>';
      }
  ...
  function attachTripEvents() {
    document.querySelectorAll('.join-trip-btn').forEach(btn => btn.addEventListener('click', function(e) { e.stopPropagation(); openTripApplyModal(this.dataset.id, this.dataset.title); }));
    if (window.isAdmin) {
      document.querySelectorAll('.edit-btn').forEach(btn => btn.addEventListener('click', function(e) { e.stopPropagation(); openTripModal(this.dataset.id); }));
      document.querySelectorAll('.delete-btn').forEach(btn => btn.addEventListener('click', function(e) { e.stopPropagation(); if (confirm('Видалити маршрут?')) { Utils.deleteItem('trips', this.dataset.id).then(() => loadData()); } }));
    }
    document.querySelectorAll('.trip-card').forEach(card => {
      card.addEventListener('click', function(e) {
        if (e.target.closest('.join-trip-btn') || e.target.closest('.edit-btn') || e.target.closest('.delete-btn')) return;
        openTripHistoryModal(this.dataset.id);
      });
    });
  }
  ...
  function closeHistoryModal() {
    document.getElementById('tripHistoryModal').classList.remove('active');
  }
  ...
  function closeLeaderTripsModal() {
    document.getElementById('leaderTripsModal').classList.remove('active');
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('addTripForm')?.addEventListener('submit', function(e) {
      ...
      if (editingTripId) {
        Utils.updateItem('trips', editingTripId, data).then(response => {
          if (response.success) {
            Utils.showNotification('Маршрут оновлено!', true);
            closeTripModal();
            loadData();
          } else {
            Utils.showNotification('Помилка оновлення', false);
          }
        }).catch(err => {
          console.error(err);
          Utils.showNotification('Помилка оновлення', false);
        ...
        Utils.addItem('trips', data).then(response => {
          if (response.success) {
            Utils.showNotification('Маршрут додано!', true);
            closeTripModal();
            loadData();
          } else {
            Utils.showNotification('Помилка додавання', false);
          }
        }).catch(err => {
          console.error(err);
          Utils.showNotification('Помилка додавання', false);
    ...
    if (addLeaderBtn) addLeaderBtn.addEventListener('click', () => { document.getElementById('leaderModalOverlay').classList.add('active'); });
    document.getElementById('closeLeaderModalBtn')?.addEventListener('click', () => { document.getElementById('leaderModalOverlay').classList.remove('active'); });
    document.getElementById('cancelLeaderBtn')?.addEventListener('click', () => { document.getElementById('leaderModalOverlay').classList.remove('active'); });
    document.getElementById('leaderModalOverlay')?.addEventListener('click', (e) => { if (e.target === e.currentTarget) document.getElementById('leaderModalOverlay').classList.remove('active'); });
    document.getElementById('addLeaderForm')?.addEventListener('submit', function(e) {
      e.preventDefault();
    ...
    document.getElementById('quickAddLeaderBtn')?.addEventListener('click', () => { document.getElementById('leaderModalOverlay').classList.add('active'); });
