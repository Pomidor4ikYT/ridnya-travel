...
(function() {
  ...
  function loadFaq() {
    ...
    if (stored && stored.length) {
      ...
      faqItems = JSON.parse(JSON.stringify(defaultFaq));
      Utils.setStorage(STORAGE_KEY, faqItems);
    }
    renderFaq();
  }

  function saveFaq() { Utils.setStorage(STORAGE_KEY, faqItems); }

  function renderFaq() {
    const container = document.getElementById('faqList');
    if (!container) return;
    if (!faqItems.length) {
      container.innerHTML = '<div class="card">Немає питань. Додайте перше.</div>';
      return;
    }
    container.innerHTML = faqItems.map(item => `
      <div class="card" data-id="${item.id}">
        <div class="faq-item-header">
          <div class="faq-item-question">${Utils.escapeHtml(item.question)}</div>
          <div class="faq-item-actions">
            <button class="icon-btn edit-faq" data-id="${item.id}"><i class="fas fa-pen"></i></button>
            <button class="icon-btn delete-faq" data-id="${item.id}"><i class="fas fa-trash"></i></button>
          </div>
        </div>
